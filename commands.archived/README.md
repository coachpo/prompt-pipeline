...
| 3 | `commands/3-tasks.md` | Decompose plan steps/epics into actionable tasks with dependencies, owners, and evidence links. | `requirement.summary`, `plan.steps` | `tasks.items`, `tasks.dependencies`, `tasks.statusSummary`, `tasks.artifacts` |
| 4 | `commands/4-qa.md` | Design verification: coverage mapping, scenarios, fixtures, tooling aligned to requirements, plan steps, and tasks. | `requirement.summary`, `plan.steps`, `tasks.items` | `testPlan.coverageMatrix`, `testPlan.scenarios`, `testPlan.fixtures` |
| 5 | `commands/5-implement.md` | Perform code + test changes per the plan/tasks/test plan and log validations. | `requirement.summary`, `plan.steps`, `tasks.items`, `testPlan.scenarios` | `implementation.changes`, `implementation.tests`, `implementation.commands` |
| 6 | `commands/6-acceptance.md` | QA sign-off: rerun suites, manual checks, record evidence/issues, update task + scenario statuses. | `implementation.changes` plus `requirement.summary`, `plan.steps`, `tasks.items`, `testPlan.scenarios` | `qaFindings.status`, `qaFindings.evidence`, `qaFindings.issues` |
